...
map<- get_map(location=c(-0.09000, 51.520), maptype="roadmap", zoom=14)
map<-ggmap(map)
map<- map+
  geom_path(data=CityofLondon_df, 
            aes(x=long,y=lat, 
                group = group), 
            color = 'darkblue', 
            fill='white', size = 1)+
...
when is about to edit the final map, in R is a very long way, there is no graphical help to do that
